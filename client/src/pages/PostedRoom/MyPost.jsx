  ...
  const handleUserRoom = async (page) => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_APP_URL}/api/room/userRoom/${id}/${page}?size=6`
      );
      setRoom(data);
        ...
        {room?.rooms?.length !== 0 ? (
          room?.rooms.map((item) => {
            return (
              <div
                key={item._id}
                className="border flex flex-col justify-center items-center p-2 hover:bg-[#f5f5f5]"
              >
                <Cards item={item} />
