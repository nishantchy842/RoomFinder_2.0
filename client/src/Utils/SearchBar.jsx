...
const SearchBar = () => {
  ...
  const onSearch = async (value) => {
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_APP_URL}/api/room/search/${value}`
      );
  ...
  return (
    <>
      <div
        className={`${styles.paddingX} w-full flex items-center mt-24 py-5  top-0 `}
      >
        <div className=" w-full  justify-center items-center max-w-7xl mx-auto hidden sm:flex">
          <Search
            className="input"
            placeholder="Enter an address, city or district ..."
            allowClear
            enterButton={
